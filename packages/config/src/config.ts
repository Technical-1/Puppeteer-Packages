...
export interface ConfigField<V = string> {
   ...
   * `ConfigError`. "Missing" means unset OR an empty string — a blank
 ...
 * `ConfigError` whose `context` carries the offending env var NAME (never
  ...
  for (const key of Object.keys(schema)) {
    ...
    if (missing) {
      if (field.required && field.default === undefined) {
        throw new ConfigError(`Missing required config: ${field.env}`, {
          retryable: false,
